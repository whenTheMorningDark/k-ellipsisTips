import { Ref, toRefs, watch, nextTick, onBeforeUnmount, reactive } from "vue";
import { Instance, createPopper } from "@popperjs/core/lib/popper-lite.js";
...
export default function usePopper(options: IPopperOptions) {
  const {
    arrowPadding,
    emit,
    locked,
  ...
  const state: IPopperState = reactive({
    isOpen: false,
    popperInstance: null,
  });
